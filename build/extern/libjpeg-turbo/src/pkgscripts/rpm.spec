...
Name: K4A
Version: 2.0.1
Vendor: The K4A Project
URL: http://www.K4A.org
Group: System Environment/Libraries
#-->Source0: http://prdownloads.sourceforge.net/K4A/K4A-%{version}.tar.gz
Release: 20200710
License: BSD-style
BuildRoot: %{_blddir}/%{name}-buildroot-%{version}-%{release}
Prereq: /sbin/ldconfig
%if "%{_bits}" == "64"
Provides: %{name} = %{version}-%{release}, K4A = %{version}-%{release}, libturbojpeg.so()(64bit)
%else
Provides: %{name} = %{version}-%{release}, K4A = %{version}-%{release}, libturbojpeg.so
%endif
...
LJT_DOCDIR=/media/min/d84b4767-6283-4b67-89ca-ed3fc4849fd5/azurekinect/Azure-Kinect-Sensor-SDK/build/extern/libjpeg-turbo/install
...
LJT_DOCDIR=/media/min/d84b4767-6283-4b67-89ca-ed3fc4849fd5/azurekinect/Azure-Kinect-Sensor-SDK/build/extern/libjpeg-turbo/install
if [ "%{_prefix}" = "/opt/K4A" -a "$LJT_DOCDIR" = "/opt/K4A/doc" ]; then
...
%doc %{_docdir}/*
%dir %{_prefix}
%if "%{_prefix}" == "/opt/K4A" && "%{_docdir}" != "%{_prefix}/doc"
 %{_prefix}/doc
%endif
%dir %{_bindir}
%{_bindir}/cjpeg
